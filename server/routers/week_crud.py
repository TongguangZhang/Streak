...
async def create_week(week: week_models.Week, supabase: supabase_dep.SupabaseDep) -> week_models.WeekInDB:
    try:
        res = await week_crud_handler.create_week(week, supabase)
        return res
...
async def get_week(week_id, supabase: supabase_dep.SupabaseDep) -> week_models.WeekInDB:
    try:
        res = await week_crud_handler.get_week(week_id, supabase)
        return res
    except Exception as e:
        root_logger.error(e)
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_week(
    ...
    try:
        res = await week_crud_handler.update_week(week_id, week, supabase)
        return res
    except Exception as e:
        root_logger.error(e)
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_week(week_id: str, supabase: supabase_dep.SupabaseDep) -> week_models.WeekInDB:
    try:
        res = await week_crud_handler.delete_week(week_id, supabase)
        return res
    except Exception as e:
        root_logger.error(e)
        raise HTTPException(status_code=500, detail=str(e))
